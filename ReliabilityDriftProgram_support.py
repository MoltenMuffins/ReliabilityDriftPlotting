...
def build_database():
    '''
    Invoked by the 'Build Database' button in the GUI.
    Takes a list of files for a single RTO and filters them based on the 
    appearance of the stress_test keywords 'HTOL, THB, TH, TC, HSTL' 
    ...
    '''
    print('ReliabilityDriftProgram_support.build_database')
    stringoffiles = askopenfilenames(filetypes=(("CSV files", "*.csv"), ("All files", "*.*")))
    # if stringoffiles:
    #     try:
    #         listoffiles = root.tk.splitlist(stringoffiles)
    #         main_df, read_test_type = combinecsv(listoffiles,test_type_select)
            # plotMPIdata(read_test_type, main_df)
        #     save_location = os.path.dirname(listoffiles[0])+'/{}_boxplot/'.format(read_test_type)
        #     if not os.path.exists(save_location):
        #         os.makedirs(save_location)
        #     saveMPIdata(read_test_type, main_df, save_location)
        # except:
...
def read_database():
    '''
    Reads a database of the file format generated by build_database()
    '''
...
def drift_calculation():
    ...
    '''
    print('ReliabilityDriftProgram_support.drift_calculation')
    stringoffiles = askopenfilenames(filetypes=(("CSV files", "*.csv"), ("All files", "*.*")))
    # if stringoffiles:
    #     try:
    #         listoffiles = root.tk.splitlist(stringoffiles)
